...
void loop() {
  ...
  delay(115); //yg wireless 95, yg serial 115
...
int readSerial2() { //rx ke rx, tx ke tx kalau pake softwareserial
    //Serial.println("TOST");
  if(Serial2.available()) {
    while(Serial2.available() ) {
      Serial2.readBytes((byte*)&data, sizeof(data)); 
      //Serial.println("TEST");
    }
    Serial.print("data dari serial2 : ");
    for (byte i=0; i<5; i++){
      Serial.println(data[i].msg);
    }
    Serial.println();
